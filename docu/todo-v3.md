...
> 替代 Obsidian，自研可视化连线工具。v1/v2 用 Obsidian 验证流程，v3 内化为产品功能。

- [ ] 读取 `internal/wiki/questions/` 目录，解析 wikilink 构建图数据结构
- [ ] 图可视化渲染（cytoscape.js 或 d3-force），节点显示文件名 + tag
- [ ] 边类型可视化区分（深挖/横向/跨域 用不同颜色或线型）
- [ ] 拖拽连线：选择边类型后连接两个节点，写入目标文件的 `# 追问` section
- [ ] 断开连线：删除对应 wikilink
- [ ] 连线操作同步更新 `internal/wiki/index.md`（度数、邻接关系）
- [ ] 节点详情面板：点击节点展示题目页内容，支持跳转到原始 raw 文件
- [ ] 过滤视图：按 tag 过滤显示子图
- [ ] 孤立节点高亮（度数为 0 的节点标红提示）
