...
type InterviewService interface {
	// SetConfig 保存面试岗位和方向配置，存入 Redis，返回 interview_id。
	SetConfig(ctx context.Context, req InterviewConfigRequest) (interviewID string, err error)

	// Create 创建面试会话，返回 interview_id 和初始阶段。
	// interviewID 由 SetConfig 生成，直接传入。
	Create(ctx context.Context, interviewID, userID string) (*InterviewCreateResult, error)
...
type InterviewConfigRequest struct {
	UserID    string
	Position  string
	Direction string // go-backend / java-backend / frontend / algorithm / ai-agent
}
